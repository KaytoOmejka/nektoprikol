...
# --- АДАПТЕР АУДИО-СИГНАЛИНГА (WebRTC поверх Socket.IO) ----------------------
# ВНИМАНИЕ: точные имена событий nekto.me для голосового чата ПУБЛИЧНО НЕ
# ДОКУМЕНТИРОВАНЫ. Значения ниже — ПЛЕЙСХОЛДЕРЫ. Их нужно уточнить, сняв трафик
# Socket.IO на https://nekto.me/audiochat (DevTools → Network → WS → кадры).
# См. README, раздел «Аудиочат: что нужно выяснить».
#
# Весь обмен SDP/ICE между браузером и сервером идёт в нашем внутреннем формате
# {"description": <RTCSessionDescription>} | {"candidate": <RTCIceCandidate>}.
# Если у nekto формат другой — переводить нужно ИМЕННО ЗДЕСЬ (в _on_signal_notice
# и send_signal), не трогая браузер.

# action, которым МЫ отправляем nekto свой SDP/ICE:
AUDIO_SIGNAL_ACTION = "anon.rtcSignal"  # TODO: уточнить реальное имя action

# notice, которыми nekto присылает НАМ сигналинг собеседника:
AUDIO_SIGNAL_NOTICES = {  # TODO: уточнить реальные имена notice
    "rtc.signal",
    "rtc.offer",
    "rtc.answer",
    "rtc.candidate",
...
class NektoClient:
    ...
    async def _on_notice(self, data: Any = None) -> None:
        ...
        # Аудио-сигналинг приходит отдельными notice — заранее их имена мы не
        # знаем, поэтому матчим по набору-плейсхолдеру и пробрасываем как есть.
        if notice in AUDIO_SIGNAL_NOTICES:
            await self._on_signal_notice(notice, payload)
            return
    ...
    # ---- аудио-сигналинг (WebRTC) ----
    async def _on_signal_notice(self, notice: str, payload: dict) -> None:
        """nekto → браузер: привести сигналинг собеседника к нашему формату.

        Наш внутренний формат (его ждёт audio.js):
            {"description": {...}}  — SDP offer/answer
            {"candidate":   {...}}  — ICE-кандидат

        Ниже — ДОГАДКА о возможной раскладке полей nekto. Реальные имена полей
        нужно сверить по снятому трафику и поправить ТОЛЬКО этот блок.
        """
        signal: dict = {}
        if "description" in payload:  # уже в удобном виде
            signal["description"] = payload["description"]
        elif "sdp" in payload:  # частый вариант: {type, sdp}
            signal["description"] = {
        ...
        # если ничего не распознали — отдаём payload как есть, пусть фронт решит
        await self._notify("signal", notice=notice, signal=signal or payload)

    async def send_signal(self, signal: dict) -> None:
        """браузер → nekto: отправить наш SDP/ICE собеседнику.

        signal приходит из audio.js в формате {"description": ...} |
        {"candidate": ...}. Обёртка action ниже — ПЛЕЙСХОЛДЕР: точные имена
        полей (dialogId? signal? раздельные action на offer/candidate?) нужно
        уточнить по трафику и поправить здесь.
        """
        if not self.dialog_id:
            return
        await self._emit(
            {
                "action": AUDIO_SIGNAL_ACTION,
                "dialogId": self.dialog_id,
                "signal": signal,
            }
        )
