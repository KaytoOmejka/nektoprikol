...
(() => {
  ...
  function adoptMicTrack(track) {
    ...
    // уже сидим на этом же живом треке — ничего не делаем
    if (micSrc && nektoMicTrack === track) return;
    try {
      ...
      // когда звонок завершится и трек умрёт — сбрасываем источник, чтобы
      // СЛЕДУЮЩИЙ звонок взял свежий микрофон (иначе тумблер «отваливается»)
      track.addEventListener("ended", () => {
        if (nektoMicTrack !== track) return;
        try { if (micSrc) micSrc.disconnect(); } catch (_) {}
        micSrc = null;
        nektoMicTrack = null;
        log("микрофон из прошлого звонка закрыт — переподхвачу на следующем");
      }, { once: true });
      log("микрофон взят из звонка nekto (без getUserMedia)");
    } catch (e) {
      micSrc = null;
      nektoMicTrack = null;
      log("не смог взять микрофон из звонка nekto", e);
    }
  }
  ...
  function setMic(on) {
    ...
    // если источник «протух» (трек прошлого звонка умер) — сбросим и переподхватим
    if (micSrc && nektoMicTrack && nektoMicTrack.readyState === "ended") {
      try { micSrc.disconnect(); } catch (_) {}
      micSrc = null; nektoMicTrack = null;
    }
