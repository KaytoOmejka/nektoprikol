...
class Session:
    ...
    async def on_event(self, name: str, kind: str, **kw) -> None:
        if kind == "message":
            ...
            # Аудио-сигналинг (SDP/ICE) от собеседника стороны `name`.
            # Релеем НЕ здесь: звук микшируется/перекидывается в браузере, а
            # сервер лишь доставляет сигналинг конкретному RTCPeerConnection.
            await self.send_browser(
                {"type": "signal", "side": name, "signal": kw.get("signal")}
            )
    ...
    async def signal_to(self, side: str, signal: dict) -> None:
        """Браузер сгенерировал SDP/ICE для стороны `side` → отдать в её nekto.

        Здесь side — это КОНКРЕТНОЕ соединение (RTCPeerConnection ↔ собеседник
        стороны side), поэтому шлём напрямую этому клиенту, без партнёрской
        инверсии, как в тексте.
        """
        c = self.clients.get(side)
        if c and signal:
            await c.send_signal(signal)
        ...
        async for msg in ws:
            ...
            try:
                if t == "connect":
                    ...
                    # Аудио: SDP/ICE из браузера для конкретной стороны.
                    await session.signal_to(data.get("side", "1"), data.get("signal") or {})
                elif t == "relay":
...
def build_app() -> web.Application:
    ...
    app.router.add_get("/audio.js", lambda r: web.FileResponse(STATIC / "audio.js"))
