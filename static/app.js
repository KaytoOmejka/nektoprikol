...
// окрасить композер под выбранную сторону («пишу как…») + крупный бейдж
function paintComposer() {
  const side = $("as").value;
  $("composer").dataset.side = side;
  const badge = $("as-badge");
  if (badge) {
    badge.dataset.side = side;
    badge.querySelector(".ab-txt").textContent = "как Собеседник " + side;
    badge.classList.remove("flip");
    void badge.offsetWidth;          // перезапуск анимации смены стороны
...
// url пустой → слот скрыт.
const PROMO = { text: "▶ ещё пранки — подпишись на канал", url: "" };
